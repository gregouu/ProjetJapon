...
<?php // Connexion à la base de données
  ...
  if($_POST) {
    ...
    $result = $pdo->prepare('UPDATE profile SET nom = :nom, description = :description, facebook = :facebook WHERE id = :id');
    ...
    while($profil = $result->fetch(PDO::FETCH_ASSOC)){ ?>  
                  ...
                  <div class="card-back card" onClick="">
                    <div class="side-c">
                        <a href="../membre/profil.php?id=<?php echo $profil['id']; ?>">
                            ...
                            <img class="side-c" src="../images/cover1.jpg" alt="photo de profile">
                        </a>
                    </div>
                  </div>
      ...
      while($listeArticles = $result->fetch(PDO::FETCH_ASSOC)){ ?>
        ...
        <form method="POST">
          ...
          <input type="text" id="nom" name="title" value="<?php echo $listeArticles["nom"]; ?>">

          <label for="author">Auteur</label>
          <input type="text" id="author" name="facebook" value="<?php echo $listeArticles["facebook"]; ?>">
